...
var formDisplayed = false; // Track if the form has been displayed
...
function drawLineBetweenMarkers() {
    ...
    var line = L.polyline([latlng1, latlng2], { color: '#d2691e', weight: 3 }).addTo(map);
     ...
     // Display the form **ONLY ONCE** after the line is drawn
    if (!formDisplayed) {
        document.getElementById("dataForm").style.display = "block";
        createToolForm("Cable");
        formDisplayed = true; // Prevent it from showing again
    }
...
function selectMarker(e) {
    ...
    // alert("Marker selected! Click 'Edit' or 'Remove' to modify.");
}
